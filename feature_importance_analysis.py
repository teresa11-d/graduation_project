...
為待開發特徵，目前以註解形式保留於程式碼中。未來資料準備齊全後取消註解即可使用。
...
使用方式:
  ...
  # 讀取真實雙 CSV 檔案
  python feature_importance_analysis.py --mode csv --projects projects_summary.csv --result my_result.csv
"""
...
FEATURE_LABELS = {
    ...
    # 待開發的文字特徵 (目前以註解呈現，待日後寫好抓取程式後可解除註解)
    ...
    # 從 CSV 解析的新增特徵 (包含字數，以及取代 has_faq 的 FAQ總數與頻率)
    "折扣層數":                 "折扣層數",
    "FAQ總題數":                "FAQ總題數",
    "FAQ更新頻率":              "FAQ更新頻率",
    "price_min":                "最低方案價格",
...
def load_from_csvs(projects_path: str, result_path: str) -> pd.DataFrame:
    df_proj = pd.read_csv(projects_path, encoding="utf-8-sig")
    df_res = pd.read_csv(result_path, encoding="utf-8-sig")
    
    if '專案編號' in df_proj.columns and 'project' in df_res.columns:
        df = pd.merge(df_proj, df_res, left_on='專案編號', right_on='project', how='outer')
    else:
        df = pd.concat([df_proj, df_res], axis=1)

    if '募資狀態' in df.columns: df[TARGET_COL] = (df['募資狀態'] == '成功').astype(int)
    else: df[TARGET_COL] = 0
        
    if '方案價格列表' in df.columns:
        price_features = df['方案價格列表'].apply(process_price_list)
...
def run(mode: str = "simulate", projects_path: str = "projects_summary.csv", result_path: str = "my_result.csv"):
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    print(f"► 成功設定圖表字型: {CJK_FONT}")

    if mode == "simulate":
        print("► 產生包含最新特徵配置的綜合模擬資料（500 筆）...")
        ...
        print(f"► 讀取並合併 CSV: [{projects_path}] & [{result_path}]")
        df = load_from_csvs(projects_path, result_path)

    print(f"  資料維度: {df.shape}，包含特徵數: {len(FEATURE_COLS)}，成功率: {df[TARGET_COL].mean():.1%}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--projects", default="projects_summary.csv", help="主專案資料檔 (預設: projects_summary.csv)")
    parser.add_argument("--result", default="my_result.csv", help="專案文案分析檔 (預設: my_result.csv)")
    args = parser.parse_args()
    run(mode=args.mode, projects_path=args.projects, result_path=args.result)